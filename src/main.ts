function firstBmi(height: number, weight: number): number {
  let bmi = weight / (((height / 100) * height) / 100);
  return bmi;
}

const getweight = function (bmi: number): string | undefined {
  if (bmi < 18.5) return "کمبود وزن";
  if (bmi >= 18.5 && bmi < 25) return " وزن سلامت";
  if (bmi >= 25 && bmi < 30) return " اضافه وزن";
  if (bmi >= 30) return "چاقی";
};
  ...
  calculateBtn.addEventListener("click", function () {
    if (height.value.trim() !== "" && weight.value.trim() !== "") {
      ...
      if (
        ...
        result.textContent = `نتیجه تست شما:${bmi.toFixed(2)} (${getweight(
          bmi
        )})`;
      } else {
